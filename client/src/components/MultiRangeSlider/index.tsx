...
const MultiRangeSlider = (props: Props, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element => {
	...
	const subSteps = props.subSteps === undefined || props.subSteps === null ? false : props.subSteps;
	const stepOnly = props.stepOnly === undefined || props.stepOnly === null ? false : props.stepOnly;
	...
	const onLeftThumbMousedown: React.MouseEventHandler = (e: React.MouseEvent) => {
		...
		const barValue = minValue;
		setIsChange(false);
		const onLeftThumbMousemove: { (e: MouseEvent): void } = (e: MouseEvent) => {
		...
		const onLeftThumbMouseup: { (e: MouseEvent): void } = (e: MouseEvent) => {
			setIsChange(true);
	...
	const onLeftThumbTouchStart = (e: React.TouchEvent) => {
		...
		const barValue = minValue;
		setIsChange(false);
		const onLeftThumbToucheMove: { (e: TouchEvent): void } = (e: TouchEvent) => {
		...
		const onLeftThumbTouchEnd: { (e: TouchEvent): void } = (e: TouchEvent) => {
			setIsChange(true);
	...
	const onRightThumbMousedown: React.MouseEventHandler = (e: React.MouseEvent) => {
		...
		const bar = thumb.parentNode as HTMLDivElement;
		const barBox = bar.getBoundingClientRect();
		const barValue = maxValue;
		setIsChange(false);
		const onRightThumbMousemove: { (e: MouseEvent): void } = (e: MouseEvent) => {
			const clientX = e.clientX;
			const dx = clientX - startX;
			const per = dx / barBox.width;
			let val = barValue + (max - min) * per;
		...
		const onRightThumbMouseup: { (e: MouseEvent): void } = (e: MouseEvent) => {
			setIsChange(true);
	...
	const onRightThumbTouchStart = (e: React.TouchEvent) => {
		...
		const barValue = maxValue;
		setIsChange(false);
		const onRightThumbTouchMove: { (e: TouchEvent): void } = (e: TouchEvent) => {
			const clientX = e.touches[0].clientX;
			const dx = clientX - startX;
			const per = dx / barBox.width;
			let val = barValue + (max - min) * per;
		...
		const onRightThumbTouchEnd: { (e: TouchEvent): void } = (e: TouchEvent) => {
			setIsChange(true);
	...
	useEffect(() => {
		if (refBar && refBar.current) {
			const bar = refBar.current as HTMLDivElement;
			const p_bar = bar.parentNode as HTMLDivElement;
	...
	useEffect(() => {
		...
		const triggerChange = () => {
			const result: ChangeResult = { min, max, minValue, maxValue };
		...
		const _barMin = ((minValue - min) / (max - min)) * 100;
		set_barMin(_barMin);
		const _barMax = ((max - maxValue) / (max - min)) * 100;
		set_barMax(_barMax);
