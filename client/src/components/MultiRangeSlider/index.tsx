...
//https://github.com/developergovindgupta/multi-range-slider-react-ts
type Props = {
  id?: string;
  min?: number | string;
  max?: number | string;
...
const MultiRangeSlider = (
  ...
  const onLeftThumbTouchStart = (e: React.TouchEvent) => {
    ...
    const barValue = minValue;
    setIsChange(false);
    const onLeftThumbToucheMove: { (e: TouchEvent): void } = (
      e: TouchEvent
    ) => {
  ...
  const onRightThumbMousedown: React.MouseEventHandler = (
    ...
    setIsChange(false);
    const onRightThumbMousemove: { (e: MouseEvent): void } = (
      e: MouseEvent
    ) => {
      const clientX = e.clientX;
      const dx = clientX - startX;
  ...
  const onRightThumbTouchStart = (e: React.TouchEvent) => {
    ...
    const barValue = maxValue;
    setIsChange(false);
    const onRightThumbTouchMove: { (e: TouchEvent): void } = (
      e: TouchEvent
    ) => {
      const clientX = e.touches[0].clientX;
      const dx = clientX - startX;
